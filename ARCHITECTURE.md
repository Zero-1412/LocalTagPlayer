...
`Architecture Baseline 0.5.30` 已在全部 Dart `part` 清零后继续收窄页面边界。Store 私有持久化协作、播放器与缩略图实现、应用服务、页面和 widgets 现在都是具有显式 import 的独立 library；`LibraryPage` 只接收页面级应用服务和必要的平台 contract，不再遍历完整组合根依赖图。
...
已完成基线：`Architecture Baseline 0.5.30`
