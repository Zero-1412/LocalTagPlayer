...
`Architecture Baseline 0.5.33` 在既有扫描与媒体探测边界上补齐大目录分阶段进度和媒体详情批处理。扫描仍先提交可用视频列表；`MediaDetailsService` 再以有限批次调用原生探测，并由 Dart Repository 合并 SQLite 写入。
...
已完成基线：`Architecture Baseline 0.5.33`
