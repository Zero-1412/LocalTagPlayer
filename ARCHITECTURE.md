...
- 首屏显式区分 `loading / ready / failed`；失败页不暴露本机路径并复用同一应用加载边界安全重试。
...
合并删除必须由用户选择保留目标，并只合并收藏和 manual 标签。启动、扫描和播放预检只能把路径
不可用的记录标记为 missing；只有用户看过数据影响并显式确认，才允许清理数据库记录与备份快照，
且清理不操作磁盘文件。临时资源使用各自清理边界。
