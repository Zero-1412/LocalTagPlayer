 ...
 *
 * 它只在显式环境变量开启时记录固定动作枚举、消息阶段、与桌面探针一致的 QPC 时间及
 * 用于事件窗口筛选的 UTC 侧车。主观察目标是 FLUTTERVIEW；顶层 runner 仅作为 Windows
 * 消息路由旁路，最终仍必须由 PlayerPage 的匿名键盘回执证明事件进入 Flutter Focus 链。
 * 不记录原始按键、媒体标识、路径或画面；正式产品不创建任何输出文件。
 */
class PlayerQaKeyboardQpcEvidence {
 public:
  ...
  /** 顶层 runner 已把消息交给 Flutter 前的旁路；只在 Debug QA 环境记录固定动作枚举。 */
