...
const _maxVisualCandidatePairs = 48;

/**
 * 内容级近重复检测器。
 *
 * 先按时长/画面规格建立有界候选对，再经 [ThumbnailService] 的 FFmpeg 取帧边界生成
 * 有序多帧 dHash。这样可以识别重新编码、容器变化或轻微裁剪后的复制品，同时不把
 * 平台路径、外部进程或解码逻辑带进页面；全部签名只在当前扫描内存中存在。
 */
class VideoContentSimilarityService {
  ...
  Future<VideoVisualScanResult> findNearDuplicateGroups(
    ...
    final videos = source
        ...
        .where((item) => item.mediaDetails?.duration != null)
  ...
  List<_VisualCandidate> _buildCandidates(
    ...
    for (var index = 0; index < videos.length; index++) {
      final duration = videos[index].mediaDetails?.duration;
    ...
    for (var i = 0; i < indexed.length; i++) {
      ...
      for (var j = i + 1; j < indexed.length; j++) {
        ...
        candidates.add(
          _VisualCandidate(left: left.index, right: right.index),
        );
  ...
  Future<List<int>?> _signatureFor(
    ...
    final duration = item.mediaDetails?.duration;
    if (duration == null || duration <= Duration.zero) {
...
class _VisualCandidate {
  const _VisualCandidate({required this.left, required this.right});
