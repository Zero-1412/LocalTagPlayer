...
extension PlayerStateDiagnostics on PlayerPageState {
  Future<PlaybackDiagnosticsSnapshot> buildDiagnosticsSnapshot() async {
    ...
    final lines = <String>[
      ...
      'GPU 高质量缩放（非 NVIDIA AI）: ${videoSuperResolutionEnabled ? '开启' : '关闭'}',
      'NVIDIA RTX 视频超分: ${nvidiaVideoEnhancementExperimentEnabled ? '会话已请求' : '关闭'} · ${nvidiaVideoEnhancementCapability.helperText}',
      'NVIDIA RTX Video HDR: ${nvidiaVideoHdrExperimentEnabled ? '会话已请求' : '关闭'} · ${nvidiaVideoEnhancementCapability.hdrHelperText}',
      'NVIDIA 自动策略: $nvidiaVideoAutomaticReason',
      'NVIDIA 滤镜互斥处理: ${nvidiaCpuEnhancementsSuspended ? '已在当前会话暂时停用压缩画质增强和暗场增强' : '未触发'}',
      '播放渲染器偏好: ${PlaybackSettings.rendererLabelFor(effectivePlaybackSettings.rendererPreference)}',
      'NVIDIA VSR 驱动确认: ${mpv['native-nvidia-vsr-state']}',
      'NVIDIA HDR 驱动确认: ${mpv['native-nvidia-hdr-state']}',
      'NVIDIA 压力保护: ${nvidiaVideoSafetyCoordinator.reason}',
      'NVIDIA 自动回滚原因: ${nvidiaVideoEnhancementRollbackReason ?? '无'}',
      'NVIDIA 自动回滚时间: ${nvidiaVideoEnhancementRollbackAt?.toIso8601String() ?? 'none'}',
      ...
      '暗部细节增强会话: ${darkSceneEnhancementActive ? '已通过 SDR/1080p/硬解门槛并启用' : '未启用 / 门槛未通过 / 已回滚'}',
      ...
      'HDR 动态映射设置: ${effectivePlaybackSettings.hdrDynamicToneMappingExperimentEnabled ? '开启' : '关闭'}',
      'HDR 动态映射会话: ${hdrMappingExperimentActive ? '已通过门槛并启用' : '未启用 / 门槛未通过'}',
      'HDR 会话压力保护: ${hdrMappingSafetyCoordinator.reason}',
      'HDR 自动回滚原因: ${hdrMappingRollbackReason ?? '无'}',
      'HDR 自动回滚时间: ${hdrMappingRollbackAt?.toIso8601String() ?? 'none'}',
