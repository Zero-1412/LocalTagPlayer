...
- 1080p / 4K Compute 帧预算已建立，两档 P95 均低于 60fps 的 25% 预留切片；第三阶段只选择 HDR 动态映射做默认关闭、确认启用、关闭恢复自动值的实验。运动补帧与时域降噪继续保持未启动。
- 下一门槛是用固定 HDR 样本建立观感、掉帧、功耗和显示输出验证，并确认运行时压力出现时的会话降级；在此之前不把实验升级为自动默认策略。
