...
- 自动协调器按实时掉帧、缓冲、停滞、FPS 与缓存余量逐级启用去块、降噪和适度锐化，并以分辨率/解码路径基线限制最高档；暗部增强仍需独立画质与性能验证。
- 第三阶段 AI 超分、时域降噪、运动补帧、HDR 映射和 Vulkan / Compute Shader 的共同前置条件改为“当前 PlayerBackend 明确报告设备/渲染能力”；未知能力不得按 GPU 品牌或型号推测为支持。
- `PlayerBackend` 的 DXGI 适配器、显存预算、D3D Feature Level、Compute 与 Vulkan 能力矩阵已经完成；多硬件卡环境若不能唯一匹配当前渲染器，第三阶段仍保持锁定。
- 下一门槛是把活动渲染适配器 LUID 从 MediaKit / 原生渲染边界显式带回，并为 1080p / 4K 建立 Compute 帧预算。之后只选择运动补帧、时域降噪或 HDR 映射中的一个做可回滚实验，不并行叠加。
- 暗部增强单独建立 SDR 暗场基线：固定黑位和暗部纹理样本，记录细节保留、黑位抬升、色带、CPU/GPU、掉帧和 UI 操作延迟；未通过前不进入自动协调器。
