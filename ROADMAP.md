...
扫描接续、进度局部刷新、动画复用与 FTS 聚合写入已完成；首次搜索与慢帧仍未达标。
输入合并与空扫描索引复用已验证，见 `docs/qa/search_delivery_index_revision_20260905.md`；后续聚焦高命中率构建与首次索引；
保留 epoch、过滤语义和播放队列，增量索引须有收益与集合一致性证据，不放宽门槛。
可丢弃 Windows 环境及无输入冲突时段就绪后验收升级、恢复与原生路径。
