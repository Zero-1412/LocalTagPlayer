...
- Agent eval 汇总增加基于用例预期的误宣告完成与缺验证记录分类；缺证据和未配置检查不计为零失败，评分与准入门槛不变。

- Agent eval 新增执行前实验身份清单、条件严格检查的 compare 和轨迹诊断；缺失条件不自动配对，重复读取只作为审查线索。

- 修复零差量扫描提交跨越搜索请求时结果停留在旧输入的问题；最新输入重新调度，旧请求仍受 epoch 校验保护。
- 扫描进度改为局部刷新，标签面板动画复用内容子树；首次搜索索引改为 SQLite 事务内聚合写入。
- 隔离 QA 增加默认关闭的索引、扫描提交和查询发布时序，支持重复启动逐轮封存及索引 SQL 分步实验；不采集搜索文本或媒体路径。
- 修复同一微任务前连续输入丢失最新搜索；成功空差量扫描复用 FTS 索引，保留查询 epoch 校验及其它内容写入的索引失效。
