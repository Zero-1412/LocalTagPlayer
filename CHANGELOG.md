...
- 当前 Windows 拖拽自动化无法保持左滑隐藏动作层，快速反向继续由 focused widget 中间态/终态覆盖；单键 `F` 被中文输入法候选截获，UI 全屏按钮路径通过。
- 点击播放器左上返回后 Debug 进程退出而非回到媒体库，窗口列表、旧句柄与只读进程检查均确认目标消失；该真实返回路径列为缺陷，不计入通过项。
