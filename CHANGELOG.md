...
- 鼠标点击进度条改用关键帧交互 seek，滑块会先保持点击目标并等待后端位置回写与新画面，避免点击后回弹和画面迟滞。

- 键盘快进/快退改为直接提交关键帧落点：短按不再在 KeyUp 触发第二次 absolute 精确 seek，也不临时静音；长按继续使用 latest-only 合并、GOP 自适应节流和临时静音，但松键只收敛最后一个关键帧预览后再恢复声音。进度条松手仍保持单次精确定位。
