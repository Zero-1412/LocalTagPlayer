...
- 备份、缓存维护动作新增 125%/150% 键盘遍历回归；标签详情使用显式视觉顺序，标签组菜单限制高度并保持字段锚点，缩放后不裁切或越界。
- 标签详情卡片改用真实 `Material` 表面，恢复开关 hover/focus/ink；合并和删除入口改为只读“检查影响”，反馈层明确未执行并默认聚焦安全返回。
- 标签编辑器添加未保存状态与精确移除 tooltip，取消仍不写入标签关系。
- 完整 224 项测试、`flutter analyze` 和 Windows debug build 通过；最新 150% 实窗因检测到用户输入停止自动点击，保留标签详情与编辑器的精确补验路径。未改 schema、过滤/队列、备份/缓存后端或用户数据。
