...
- 150% 媒体库卡片标题裁切留给媒体库响应式任务；100% 播放器已置前等待人工左滑快速反向终态截图。
...
- Windows 自动化三次左滑均只改变队列选中项，无法让隐藏动作层保持展开，快速反向以 focused widget 中间态、终态与可中断吸附断言替代；单键 `F` 被中文输入法候选截获，UI 全屏入口已通过。
