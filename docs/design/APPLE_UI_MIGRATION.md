...
- 标签组菜单使用字段同宽锚点、有限高度和维护页圆角；结构表面改用真实 `Material`，保证 hover、focus 与 ink 不被底色遮住。
- 高风险动作不再用“删除”制造立即执行错觉，改为“检查影响”；只读反馈明确本次没有修改数据，默认焦点落在安全返回。标签编辑器的临时改动用文字和 live region 提示，不依赖颜色。
- focused tests、完整 224 项测试、静态分析与 Windows debug build 通过；没有引入 blur、列表重算、I/O、数据库迁移或队列重建。最新 150% 实窗补验因检测到用户输入暂停。
