...
- 单条缺失路径和批量预览路径映射直接使用原生 `Tooltip`，维护页路径信息的浮层层级不一致；
...
- 单条路径与批量预览映射统一使用 `MaintenanceTooltip`，保留完整旧路径/新路径消息和现有页面 owner；
...
| Before | After | 影响 |
| --- | --- | --- |
| 单条缺失路径、批量预览路径映射直接使用原生 `Tooltip` | 通过 `MaintenanceTooltip` 复用维护主题、实色表面和统一等待时序 | 路径信息在页面与批量预览中保持同一浮层层级，不改变稳定身份或路径内容 |

页面级 focused 测试分别确认单条路径和批量预览映射的具体 tooltip 消息已挂载。
