...
- root 路径提示直接使用原生 `Tooltip`，维护页局部浮层主题未形成统一调用证据。
...
- root 路径提示统一使用 `MaintenanceTooltip`，保留完整路径消息、键盘可达性和页面 owner 不变；
...
| Before | After | 影响 |
| --- | --- | --- |
| root 路径直接使用原生 `Tooltip` | 通过 `MaintenanceTooltip` 复用维护主题、实色表面和统一等待时序 | 统一目录管理与其它维护页的浮层层级，不改变路径内容 |

页面级 focused 测试同时确认 `MaintenanceTooltip` 已实际挂载；这不是只检查共享组件孤立存在。
