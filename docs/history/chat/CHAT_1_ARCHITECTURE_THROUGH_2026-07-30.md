...
> 历史归档；当前领域合同见 `docs/chat_tasks/CHAT_1_ARCHITECTURE.md`。
...
- Inno Setup 只声明简体中文，关闭语言选择与旧安装语言复用，避免历史英文安装记录影响
  新版本。快捷方式与安装后启动描述改用语言包常量。
- 发布工作流从 Inno Setup 官方仓库固定提交下载 `ChineseSimplified.isl`，核对固定
  SHA-256 后才编译；下载失败或内容变化会直接中止发布，不能静默回退英文。
- 本机没有 ISCC，无法生成正式安装页面截图；下一次 Windows Release 必须补做安装、
  覆盖安装、关闭运行中应用、完成和卸载路径的中文文案预览。
- 安装目录、应用数据目录和卸载时保留用户数据的既有行为未改变。
...
- 前一批架构基线 `0.5.133` 的完整审计和 Phase 0-6 路线记录在
  `docs/history/architecture/ARCHITECTURE_REFACTOR_2026_07_29.md`。
