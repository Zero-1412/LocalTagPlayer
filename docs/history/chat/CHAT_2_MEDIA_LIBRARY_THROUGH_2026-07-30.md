> 历史归档；当前领域合同见 `docs/chat_tasks/CHAT_2_MEDIA_LIBRARY.md`。
...
- SQLite schema、`FilterQuery` / `TagQueryService` 语义、manual 标签和 filtered queue 来源不变；完整性能证据见 `docs/history/qa/2026-07/library_add_remove_player_stress_20260714.md`。
...
- 路径当前不存在时直接纳入清理，不要求扫描先标记 missing；存在但不可读必须是无法作为普通文件打开，不能用缩略图或 FFprobe 失败代替。
