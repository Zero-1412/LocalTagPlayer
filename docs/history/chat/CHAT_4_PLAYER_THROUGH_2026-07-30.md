> 历史归档；当前领域合同见 `docs/chat_tasks/CHAT_4_PLAYER.md`。
...
- `PlayerShortcutFeedback` 的实色浮层改为 38% 不透明黑色背景、18% 白色描边和纯白前景，
  让视频内容保持可见；没有引入 `BackdropFilter`、动态 blur 或新的动画控制器。
- 高对比度模式使用 78% 背景和 52% 描边，普通与高对比度材质均由 widget 测试固定。
- 左上角挂载、16 dp 安全区、850ms Timer、语义、指针穿透和所有播放器命令路径未迁移。
- 完整 484 项测试、静态分析、Windows Debug build 与可见窗口启动门禁通过；真实窗口
  暂停提示截图确认亮白文字/图标、半透明背景、轮廓、对齐和中央空白均正常。
...
- 视频画面 `GestureDetector` 新增双击播放/暂停，复用键盘动作的同一反馈方法；
  右键菜单、单击取回焦点、滚轮输入、控制条和队列条目双击播放均保留。
- 未被输入门禁或用户自定义动作消费的回车/小键盘回车切换全屏；现有 `F` 或用户配置
  仍可用。播放/暂停与全屏忽略重复事件，长按不会造成状态抖动。
- 快进/快退继续接收 `KeyRepeatEvent`。协调器把最近后端提交计时提升到实例级，
  防止长按期间工作器快速结束后绕过 80ms 节流；最终累计目标仍保证提交。
- focused 合同固定画面双击与回车页面挂载，并保护队列双击入口；长按专项用 10 次
  15ms 重复输入验证中间合并、提交间隔和最终 50 秒目标。
- 完整 481 项测试、静态分析、Windows Debug build 和精确 PID 双击启动门禁通过；
  Computer Use 因用户正在操作目标窗口停止，准确人工点击路径记录在 `CURRENT_TASK.md`。
- filtered queue 来源/顺序、当前 index、PlayerBackend、缓存队列和用户数据均未改变。

## 2026-07-30 连续快进快退画面跟随

- 根因是页面原有 80ms 纯尾随防抖：连续输入期间只更新文字目标，不向后端发送 seek，
  因而画面必须等用户停手后才跳转。
- 新增纯应用层 `PlayerSeekCoordinator`。每轮首次目标立即提交；连续输入以 80ms 最小
  间隔串行刷新最新累计目标，后端繁忙时合并旧目标，停止输入后仍保证提交最后目标。
- 相对快进/快退基于尚未确认的最新目标累计，避免连续操作回退到滞后的后端位置；
  `PlayerBackend`、MediaKit/Windows MPV 实现、filtered queue 来源和播放器索引均未改。
- 确定性测试覆盖首次即时提交、繁忙期间合并、累计目标和时长边界。真实 MediaKit
  Texture 门禁覆盖 40ms 连续输入、最终位置、帧摘要变化和掉帧上限。
- 既有完整稳定性矩阵超过外层 7 分钟且未生成报告，不计为通过；新增轻量真实门禁
  38.6 秒通过。479 项测试、静态分析与 Windows Debug build 均通过。
...
- 完整证据见 `docs/history/qa/2026-07/player_native_output_size_gate_20260730.md`；队列来源、当前
...
- 完整口径见 `docs/history/qa/2026-07/player_texture_sampling_ab_20260730.md`；filtered queue、
...
- 播放诊断补充缩小器/缩小校正读回；可复测脚本、集成模式与完整口径见
  `docs/history/qa/2026-07/player_downscale_quality_ab_20260730.md`。
...
- 播放器顶栏删除重复队列按钮；底部控制条统一控制宽屏、紧凑和全屏队列。紧凑布局由
  居中 `BottomSheet` 改为右侧覆盖层，遮罩和关闭入口不改变列表/详情统一侧栏内容。
- 修复小窗口下底部按钮只切换未挂载宽屏状态的失效链路；新增页面挂载合同，禁止入口
  与侧栏再次脱节。filtered queue、当前/选择索引、子标签、收藏、删除及返回路径不变。
- 本机运行时读回 GPU 高质量缩放为 `ewa_lanczossharp` / `lanczos`，实际硬解为
  `d3d11va-copy`；当前 1080p60 SDR 样本在 988×694 窗口中没有上采样，压缩增强又因
  高帧率 copy-back 基线关闭，HDR→SDR、插值和暗部增强也未对该片源生效。
- focused 测试、完整 460 项测试、静态分析与 Windows Debug build 通过，3 项既有
  benchmark 跳过。真实小窗口点击底部入口、右侧展开和显式关闭均通过截图检查。
...
- `player_settings_panel.dart` 降至 417 行，一级、高级与选项展示分别独立；设置确认、
  循环模式、画面比例、倍速、压缩增强和快进档位回调仍由播放器页面 owner 执行。
- `player_control_slider.dart` 收敛为兼容入口，主进度交互、通用表面、矢量形状与指标
  分离；延迟帧预览、拖动结束提交 seek、隐藏进度线和全屏焦点回归通过。
- `player_queue_list_item.dart` 降至 497 行，滑开后的收藏/删除区和详情文案迁为纯展示
  叶子；来源 filtered queue、选择索引、收藏和删除命令未迁移。
...
- 证据见 `docs/history/qa/2026-07/player_fvp_same_method_windows_ab_20260729.md`。
...
- Windows MPV 视频宿主始终使用完整 Flutter 占位矩形；底部控制条显示/隐藏由 native
  region 在 128 与 3 逻辑像素间切换，不再留下固定黑区。
- runner 先更新本轮 surface 几何再计算 region，修复首帧错位和底部重复条带。
- 右键菜单使用正确的视口剩余边距，并在 PopupMenu Route 挂载后有限重试测量。
- 设置与右键弹层期间不截图、不冻结、不隐藏整个 HWND；覆盖区外视频持续播放。
- filtered playback queue、当前 index、队列切换和返回路径保持不变。
...
- 证据见 `docs/history/qa/2026-07/mpv_hwnd_overlay_region_20260728.md`。
...
- 完整证据见 `docs/history/qa/2026-07/mpv_nvidia_native_d3d11_20260727.md`；专业播放器功能
  调研与后续插值/补帧/着色器路线见
...
- DPR 已纳入几何同步判定并通过 100%→150% focused test；当前测试机只有单个
  ...
  三类片源六组 A/B。
- 完整记录：`docs/history/qa/2026-07/child_hwnd_airspace_20260727.md`。
...
- 任何 HWND 原型必须保护 filtered queue、当前 index、返回状态、控制条、
  ...
  通过前，不替换默认 MediaKit。
- 完整记录：`docs/history/qa/2026-07/angle_d3d11_interop_20260727.md`。
...
- 完整记录：`docs/history/qa/2026-07/mpv_nvidia_scaling_isolated_20260727.md`。
...
- 评估记录见 `docs/history/qa/2026-07/rtx_video_sdk_feasibility_20260727.md`；本轮不修改 `PlayerBackend` contract、filtered queue、解码器、缓存队列或用户数据。
...
- 新增 `tool/run_natural_compression_quality_ab.ps1` 与 `docs/history/qa/2026-07/player_natural_compression_ab_20260727.md`；正式播放器、filtered queue、最新请求切换、硬解回退、超分/HDR/暗部增强、缓存队列和用户数据均未修改。

## 2026-07-27 压缩画质增强与保守去色带

- 播放器齿轮一级增加“压缩画质增强”，三级列表明确提供“关闭 / 自动 / 清晰增强”；页面级挂载合同同时保护齿轮、回调、子页和三个稳定选项键。
- `PlaybackSettings` 用枚举持久化三档，并继续写入旧布尔键供降级版本读取；旧配置 `true` 迁移为自动、缺失或异常值回到关闭。
- 自动档保留既有自适应升级与立即回滚；清晰增强首次健康样本请求当前 profile 的最高安全档，压力发生后恢复为原迟滞升级，避免反复拉高和振荡。
- 去块、`hqdn3d`、`unsharp` 继续以单一 `vf` 图应用；增强档额外启用保守 mpv GPU 去色带，所有属性写入串行并容忍单属性不可用，关闭时先停用去色带再清空滤镜。
- 450 kbps 低码率 1080P 固定样本完成关闭/清晰增强各 20 秒实机基线，两档 0 掉帧、0 音视频卡顿；真实窗口固定到同一 12 秒暂停帧，三档菜单无遮挡、选中反馈明确。
- 局部放大未发现明显锐化光晕，现有轻锐化已经覆盖主要收益；缩放后 GLSL 锐化本轮不加入。filtered queue、最新请求切换、硬解回退、超分/HDR/暗部增强、返回状态和用户数据均保留。
...
- 细进度恢复到视频表面根 Stack 的底部独立层，与完整控制条使用互斥淡入；控制条透明或位移收起不会再连带删除最低限度的播放位置反馈。
- Debug 真实点击完成“进入播放器 → 自动隐藏 → 细进度保留 → 底部唤回完整控制条”两态复测，截图位于 `.local/qa/player-hidden-progress/`；filtered queue、播放器后端与缓存边界均未改变。

## 2026-07-22 快速切换与预览纹理交接

- 媒体卡动态预览使用独立 MediaKit Player，但媒体库 Route 会在播放器下方保留；卡片点击现先停止预览媒体输出、取消旧 open 代次并释放纹理，再进入正式播放器，避免两套 Windows 视频输出同时切换分辨率。
- 首帧缩略图只用于首次进入播放器的冷启动接管，后续队列切换不再替换缩略图图片流；当前 filtered queue、播放索引和队列定位语义不变。
- latest-request worker 在滤镜、性能配置、`openPath` 和首帧校验的异步边界后检查新请求；更新选择到达时立即跳过旧路径。损坏媒体判定仍约 1.5 秒，新选择检测粒度从 250ms 缩短到 80ms。
- Windows 日志确认修复前存在 `flutter_windows.dll` 原生异常；修复后真实预览纹理在正式纹理创建前完成释放，跨 1080p / 2048×1080 / 4K 共 40 次切换无新增 Application Error。
- 20 次毫秒级输入用时 4.19 秒且只产生 5 次实际 open，证明中间旧请求被合并而非逐条等待播放；完整 264 项测试、静态分析和 Windows Debug build 通过。
- `PlayerBackend` contract、SQLite schema、标签语义、filtered queue 来源/顺序、缓存队列、稳定身份和用户数据均未改变。

## 2026-07-22 控制条显隐、全屏覆盖队列与快进文字水印

- 控制条进入播放器时默认显示，3 秒无交互后自动隐藏；隐藏后仅底部进度区域唤出，显示/隐藏使用可中断的淡入与位移，并服从 reduced motion。
- 全屏队列挂到播放器根 `Stack`，以短距离右滑和淡入覆盖视频右侧、铺满当前层级高度；视频纹理不随队列开合重新测量，11176 项滚动仍保持虚拟列表和独立滚动状态。
- `PlaybackSettings` 新增 5 / 10 / 15 / 30 / 60 秒快进快退档位，更多播放设置用离散滑动条维护；快捷键连发只触发一次左上角“前进/后退 N 秒”文字水印，不再占用画面中央。
- Windows 实窗发现设置页内部 `AnimatedSwitcher` 同时保留新旧复杂树时会和视频纹理一起触发引擎访问冲突；内部页面改为直接替换后连续打开“更多播放设置”稳定，外层弹层动效保持。
- 1249×714 / 2560×1440 真实点击覆盖自动隐藏、左上角 seek 水印、设置滑动条、全屏队列开合及滚动；完整 238 项测试、静态分析和 Windows Debug 构建通过，3 项显式 benchmark 跳过。
- filtered queue 来源、内容、顺序和当前 index 未改变；`PlayerBackend`、缓存队列、SQLite schema、标签语义、稳定身份与用户数据均未改变。

## 2026-07-21 队列定位语义与卡片重命名入口

- 队列顶部删除重复的“回到选中”图标，底部只在目标离开视口时按需显示“回到选中”或“回到播放”，同一动作不再同时占用两个位置。
- `playingIndex` 继续表示真实播放事实，`selectedIndex` 表示用户浏览焦点；“回到播放”只读取播放索引并滚动，不再调用选择动作。实窗从选中项 16 回到播放项 4 后，底部立即恢复“回到选中”，证明浏览焦点未被覆盖。
- 媒体库网格、列表与本地目录卡片通过“更多 → 重命名文件”复用播放器现有弹窗和路径事务；改名会刷新文件名关键字结果与排序，但不重算未变化的标签计数。
- 隔离临时文件冒烟真实执行确认改名，模拟 Windows sharing violation 后验证 pause/stop、第二次改名、open 新路径、seek 原位置和恢复播放；同一 `videoId`、manual 标签与收藏保持。
- 完整 235 项测试、静态分析、Windows debug build 和 1249×714 实窗通过；顶部工具栏、底部动作、卡片菜单与弹窗无重叠、遮挡、错位或溢出，真实媒体改名已取消。
- filtered queue 来源、内容、顺序、当前播放索引和 `PlayerBackend` contract 未改变；SQLite schema、标签过滤、缓存队列与用户数据语义不变。
...
- 设备矩阵可由 `tool/run_gpu_capability_matrix.ps1` 重建，结果口径见 `docs/history/qa/2026-07/player_gpu_capability_matrix_20260722.md`。
...
- 完整口径见 `docs/history/qa/2026-07/player_hdr_sdr_baseline_20260722.md`。filtered queue 来源/内容/顺序、当前 index、SQLite、标签语义、稳定身份、缩略图/媒体详情队列与用户数据均未改变。
...
- 播放器交互页删除热区宽度、隐藏延迟和恢复默认控件，只保留默认开启的“全屏边缘播放列表”开关；参数属于实现容错，不再作为普通用户决策暴露。
- 自动唤出使用固定 32px 右缘入口；展开后以实际 320–476px 列表宽度加 12px 容错保持，真正离开后等待 450ms 再隐藏。
