...
> 历史归档；当前领域合同见 `docs/chat_tasks/CHAT_6_TAG_MANAGER.md`。

## 2026-07-29 标签管理 500 行警戒线治理

- `tag_manager_page.dart` 519→499，标题栏迁为 58 行快照/回调组件；页面继续持有搜索、
  分组、选中详情、usage 刷新与所有维护命令。
- manual-only 批量增删、folder 来源保护、引用检查、稳定 `tagId` 和用户数据语义不变；
  架构合同、完整测试与静态分析通过。
...
- 详情按使用情况、标签属性、批量打标签和高风险操作分组；批量操作仍只允许 manual，folder 仍由路径派生，合并/删除继续先检查引用关系。
- focused tests 覆盖搜索真实输入/清除和分组选择；完整 200 项测试、静态分析和 Windows debug build 通过。真实窗口因用户前台游戏占用而停止控制，精确复测路径记录在 `CURRENT_TASK.md`。
- SQLite schema、标签模型、删除/合并实现、当前筛选结果和用户数据均未改变。
