...
> 历史归档：此方案已经执行，不应再作为当前任务输入；当前边界以根目录 `ARCHITECTURE.md` 为准。
...
- [x] `library_page.dart` 从 4293 行降到 750 行，只保留 Route 注入和 Widget 编排。
- [x] 生命周期、扫描、导航、最近播放、查询、筛选、Route、播放和用户命令按一致性边界
  拆为独立 mixin；运行时引用与跨 mixin 契约独立集中，但不复制任何业务 owner。
- [x] 设置状态/命令 owner 与纯展示工作区拆为 435/252 行；展示区只消费快照和回调。
- [x] 页面低于 1000 行、所有新协调文件低于 500 行成为架构合同；旧交互清单继续验证
  真实挂载源。schema、查询、filtered queue、缓存队列与用户数据未改变。
...
详细决策、停止条件和重新评估证据见
[`ADR_001_PROGRESSIVE_ARCHITECTURE_MIGRATION.md`](../../architecture/ADR_001_PROGRESSIVE_ARCHITECTURE_MIGRATION.md)。
