...
> 本文件是截至 2026-07-30 的完整变更归档；当前变更入口见仓库根目录 `CHANGELOG.md`。
...
- 左上角操作提示改用 38% 不透明黑色背景、弱白色描边和纯白文字/图标，降低对视频内容
  的遮挡；不使用 blur。
- 高对比度模式自动提高背景与描边不透明度，保持复杂画面上的可读性。
- 位置、尺寸、850ms 生命周期、首次触发、连续 seek、指针穿透和实时语义保持不变。
- 完整 484 项测试通过、3 项既有 benchmark 跳过；静态分析、Windows Debug build、
  可见窗口启动和真实暂停提示截图验收通过。
- PlayerBackend、filtered queue、SQLite、标签、缓存队列和用户数据均未改变。

## 2026-07-30 · 统一播放器操作提示位置

- 播放器的音量、播放状态、全屏、倍速、切换视频和 seek 反馈统一显示在视频左上角，
  中央不再显示任何操作提示，首次触发同样走左上角。
- 移除快进/快退专用中央水印和对应状态分支，保留既有反馈文案、图标、850ms 生命周期、
  无障碍实时区域及连续 seek 行为。
- 页面级挂载合同与 widget 坐标测试保护单一左上角反馈入口。完整 483 项测试通过、
  3 项既有 benchmark 跳过；静态分析、Windows Debug build、可见窗口启动及真实音量/
  快进点击截图验收通过。
- PlayerBackend、filtered queue、SQLite、标签、缓存队列和用户数据均未改变。

## 2026-07-30 · 完善播放器鼠标、回车与长按 seek

- 视频画面双击切换播放/暂停；播放器主焦点下回车和小键盘回车切换全屏，原有可配置
  快捷键、输入门禁和队列条目双击播放保持可达。
- 长按快进/快退的重复键事件持续累计目标；最近提交间隔跨工作器保留，后端快速确认也
  不能绕过 80ms 节流，松键前最终目标不会丢失。
- 播放/暂停和全屏动作忽略重复键事件，避免长按造成连续状态反转。
- 修复当前 Debug 交付目录：根因是 Windows integration test 残留测试宿主入口；全部
  测试后重新构建 `main.dart`，精确 PID 双击启动取得可见主窗口。
- 完整 481 项测试通过、3 项既有 benchmark 跳过；静态分析、Windows Debug build 和
  双击启动门禁通过。真实交互截图因检测到用户输入而安全停止，人工路径已记录。
- PlayerBackend、filtered queue、SQLite、标签、缓存队列和用户数据均未改变。
...
- GitHub Release 支持 Range 时采用四段并行下载，八连接因实测出现连接重置而未启用；服务端忽略
  Range 时自动回退单流，瞬时网络错误允许每段重试一次。
- 更新弹窗显示实时速度与预计剩余时间；同版本已完成且 SHA-256 校验通过的安装包可以直接复用。
- 分段合并后继续执行发布清单 SHA-256 门禁，失败时清理全部临时文件，不降低安装包完整性要求。
- 四连接样本约 463 KiB/s，单连接约 54 KiB/s；完整 476 项测试通过、3 项既有 benchmark
  跳过，静态分析零问题，Windows Debug build 成功。
- schema、标签、筛选语义、filtered queue、播放器后端、缓存队列与用户数据均未改变。
...
- filtered queue、PlayerBackend 基础 contract、SQLite、标签、缓存队列和用户数据
  均未改变。详细记录：`docs/history/qa/2026-07/player_native_output_size_gate_20260730.md`。
...
- 详细记录：`docs/history/qa/2026-07/player_texture_sampling_ab_20260730.md`。
...
- 删除播放器顶栏重复的播放队列按钮；底部控制条统一控制宽屏常驻侧栏、紧凑右侧覆盖层
  和全屏队列。高 DPI/小窗口不再打开居中 `BottomSheet`，右侧覆盖层支持遮罩和显式关闭。
- 页面级合同固定底部入口与紧凑侧栏挂载，禁止队列组件孤立或退回错误位置；filtered
  queue 来源、当前/选择索引、列表/详情、搜索、收藏、删除和返回媒体库状态均未改变。
- 运行时审计确认同一 MediaKit NativePlayer 已读回 GPU 高质量缩放属性；当前 1080p60
  SDR 样本在小窗口内是缩小显示，压缩增强又因高帧率 copy-back 基线保守关闭，因此低
  画质观感不明显不是属性写入失败。未盲目开启不适用或相互冲突的 mpv 高级属性。
- 完整 460 项测试通过，3 项既有 benchmark 跳过；静态分析与 Windows Debug build
  通过。真实 988×694 Debug 窗口点击底部按钮后队列从右侧展开，关闭正常且布局无异常。
...
- Architecture Baseline 提升到 `0.5.134`：`library_page.dart` 从 4293 行拆到 750 行，
  ...
  1000 行强制重构清单。
- 新架构合同要求页面外壳保持低于 1000 行、协调文件保持低于 500 行，并对整个页面
  协作簇继续验证查询、计数、扫描、文件命令、稳定身份队列与设置 owner；旧交互清单的
  播放器 Route 挂载源更新到播放 mixin，未授权功能删除为零。
- 架构合同 51 项、媒体库/设置 focused 212 项及完整 452 项测试通过（3 项 benchmark
  跳过），静态分析、Windows Debug 构建和正式入口点击启动门禁通过。Computer Use
  原生管道仍不可用，人工点击路径记录于 `CURRENT_TASK.md`。
- 前一批 Architecture Baseline `0.5.133` 拆出标签 helper、选择工具栏、顶栏附件控件
...
- 完成 Phase 2E 现有数据备份设置纵向切片：提取通用串行设置 owner、备份状态订阅
  owner、维护命令互斥 owner 和 `DataBackupSettingsWorkspace`。设置文件失败时的运行态
  回滚、只读完整性检查、导出文件选择与写入继续留在应用服务、Repository 和平台
  adapter；源码没有数据库关闭/替换/重开或导入入口，因此未新增破坏性恢复。
- 备份入口、卡片、开关、三个动作和返回路径均由页面级挂载测试保护，媒体库页面预算
  降到 6,021。完整 350 项测试通过（3 项显式基准跳过），静态分析、Windows Debug
  build 和正式入口可见窗口启动通过；自动点击/截图仍受 Computer Use 原生管道不可用
  阻塞，人工路径记录于 `CURRENT_TASK.md`。
- 完成 Phase 3A/3B 媒体库轻量状态收口：独立结果数据/标签定义修订 tracker，普通内容
  更新不再无意义推进标签定义代次；root、扫描、relink、删除与标签维护仍保守失效标签
  候选。主结果多选只保存 stable `videoId`，网格、侧栏和标签面板显隐由纯展示 owner
  管理，页面继续持有筛选、排序持久化、批量删除和复合 rebuild。
...
- 完成 Phase 3G-1 文件菜单 command：定位、改名和删除使用显式不可变输入与同一无 UI
  executor；页面继续拥有确认层、删除偏好、SnackBar 和刷新时机。
...
- Windows 原生 MPV 把成组属性合并为一次平台调用和一次状态采样；工作线程在每条
  控制命令前优先消费待渲染帧，避免打开媒体、切换视频和设置属性时长期饿死渲染。
- 单条命令直接返回状态快照，不再紧接一次额外轮询；打开媒体、GPU 缩放、HDR、
  平滑运动与压缩增强均复用可选批处理边界，其他后端保持串行回退。
- 50/60fps `d3d11va-copy` 会话（含 1080p60、4K60）保守关闭 CPU 滤镜；
  低帧率自动增强一旦确认新增掉帧，当前媒体锁定关闭，消除
  “升档→掉帧→回滚→再升档”的循环。
- 诊断只在连续两次位置推进不足或存在掉帧、停滞、缓冲、AV 偏移等独立证据时告警，
  并补报原生 Texture 后端可用的总掉帧增量。
- 同一 4K60 样本修复前 19 秒新增 49 个掉帧、AV 偏移约 0.35 秒；修复后连续约
  49 秒总掉帧保持 1、AV 偏移约 0.000004 秒、表面重建保持 6。真实队列收放、
  连续 seek 与快速切换均通过。
- SQLite schema、标签过滤、filtered queue、MediaKit、缓存队列和用户数据未改变。

## 2026-07-28 · 全应用功能与动画对抗式压测

- 主窗口语义压力覆盖搜索、六种排序、标签父子选择、面板开合和网格/列表；真实窗口完成
  12 轮视图切换与 30 次设置子页往返，没有布局残留、遮挡或路由丢失。
- MediaKit / MPV 各完成 6 次全屏往返、100 次快速切换和 120 秒长播；停滞均为 0，
  最大掉帧 0/2，队列、设置和 seek P95 均低于 23ms。
- 网格卡片补齐稳定播放语义；压力脚本改为重新解析语义节点并遵守标签选择后的自动折叠，
  不再依赖过期索引。
- MPV 进入原生全屏前等待 Flutter 提交“非全屏顶栏已卸载”帧，消除 child HWND
  尺寸切换后残留的顶部序号/筛选摘要像素；真实全屏与全屏队列稳定帧通过。
- 307 项测试、静态分析和 Windows Debug build 通过。SQLite schema、标签过滤语义、
  filtered queue、缓存队列与用户数据未改变；真实跨物理 DPI 继续作为人工发布门禁。

## 2026-07-28 · 消除 MPV Texture 交互阻塞

- 将 Flutter GPU Texture 描述符读取从整帧 MPV/ANGLE/D3D11 临界区拆出，只在共享
  句柄尺寸重建和销毁时同步，消除列表、设置弹层与 seek 期间的 raster 线程等待。
- 进度拖动只在松手时提交最终 seek；键盘快速前进/后退合并为 80ms 内的最终目标，
  并基于尚未确认的目标继续累加，不再反复读取滞后播放位置。
- 渲染器保存提示在 4 秒后自动隐藏，同时保留切换确认和短时撤销；播放列表、全屏、
  右键、设置、filtered queue 和返回路径均保留。
- 三类低码率片源同条件 Debug A/B 中，列表、设置、seek 的 P95 分别由
  `47.311/52.285/505.127ms` 降至 `14.719/21.391/15.636ms`；长播停滞 0，
  最大总掉帧保持 2。
- 同一用户文件确认源范围为 limited、矩阵为 BT.709、mpv 输出策略为 auto；未加入
  全局亮度补偿。SQLite、标签过滤、缓存队列和用户数据未改变。
...
- MPV child HWND 改为完整占位矩形，控制条改由 native region 动态让出；控制条隐藏后仅保留
  3 像素进度条，消除底部永久大块空白。
- surface 新几何先于 region 生效，修复首入和切换时的画面错位、重复条带。
- 右键菜单修正 `RelativeRect` 边距并重试真实边界测量，视频在菜单外继续实时显示。
- HWND 回归测试覆盖动态上下控制区、设置/右键弹层、播放推进、零纹理复制与零掉帧。
- FFmpeg 依赖改用同系列月末保留构建，避免普通日构建过期导致干净环境无法恢复。
...
- NVOFA 前向/后向执行新增 `UINT8` 硬件 cost 输出；同 LUID D3D11 Compute
  同时读取双向光流、cost 与前后向 residual。
- 拒绝了虽通过技术门禁、却在动画翼缘制造暗色拖影的强置信度加权版本；最终
  保留中点取样和 85% 等权合成，只允许 42.5%–57.5% 的保守比例修正。
- 新增独立确定性 Compute 探针，锁定零流等权 128、正确运动 90 与极端不可靠
  单侧 117；输出标记改为 `LTPNVOFAConsistencyProtected`，避免冒充完整 FRUC。
- 真人面部、动画渐变、暗场六组 20 秒 A/B 再次全部通过：24→48fps、off/on
  总掉帧 0/0、音视频停滞 0/0，并命中 LUID `00000000:00017093`；三类开启
  截图人工复核无新增明显伪影。
- A/B 工具只续用具备报告、完整截图、成功日志且插件 SHA-256 与当前二进制
  一致的单组结果；最终六组与摘要均绑定相同 hash，失败或旧实现仍强制重跑。
  插件仍为 QA-only、无 install、无 bundle，产品入口和默认后端不变。
- 遮挡 mask、矢量补洞、图像域显露区域补洞和连续快速运动审查仍是下一阶段；
  filtered queue、插件 ABI v1、SQLite、标签、缓存与用户数据不变。
...
- 共享纹理复制前移到原生渲染线程；插件处理前由宿主备份原帧，返回失败会恢复纹理、停用插件并继续播放。
- 新增不安装的 D3D11 往返探针与宿主自测。探针无视觉改动验证同设备输入/输出，故障模式会先破坏输出再报错；自测确认无损往返与原帧恢复均通过。
- 播放诊断可见插件状态、处理帧、回退数和稳定错误码；未新增用户设置或数据库字段。
- 标准应用 bundle 不包含探针或 NVIDIA 文件；真实 RTX Video SDK 的下载包 EULA、再分发许可、厂商能力探测和多显卡验证仍是后续阻断。
- 隔离低码率 1080P 的正常/第 30 帧故障两轮 Windows 集成测试均通过；回退后播放位置继续推进、总掉帧为 0，齿轮与右键诊断弹窗无溢出或遮挡。
- SQLite schema、标签查询、filtered queue、`PlayerBackend` contract、缩略图/媒体详情队列和用户数据均未改变。
...
- 三类关闭/清晰增强共六轮 Windows 实播均为 0 掉帧、0 音视频停滞；固定屏幕帧确认现有增强克制，额外后锐化未跨三类稳定获益，因此不加入 GLSL。
- 明确 NVIDIA 边界：当前 GPU 画质超分是 libmpv 高质量缩放，不是 RTX Video AI；本机 NVIDIA 视频超分开关也处于关闭，播放器未接 RTX Video SDK。
- 正式播放器代码、SQLite、标签查询、filtered queue、PlayerBackend contract、缓存队列和用户数据均未改变。
...
- Windows 正式安装器改为在应用内下载并显示进度；下载先写 `.part`，完成后校验 GitHub 资产 SHA-256，只有摘要匹配才启动交互式安装器。
- 安装包摘要缺失、下载不完整、校验失败或当前平台不支持应用内安装时不会执行文件，并保留 GitHub Release 页面作为降级入口。
- 启动后静默检查、稍后提醒、离线不阻塞媒体库和设置页既有入口保持不变；SQLite、标签/过滤语义、filtered queue、`PlayerBackend`、缓存队列和用户数据未修改。

## 2026-07-24 · 路径失效记录纳入自动清理

- 修正自动清理条件：数据库路径当前不存在时直接清理，不再要求扫描先写入 `isMissing`。
- 回归测试覆盖“未标记 missing 但路径已失效”，并继续确认可读文件以及磁盘目录本身不会被删除。
- SQLite schema、标签过滤、播放队列和磁盘删除边界不变。

## 2026-07-24 · 远程开发分支收口

- 自动清理分支的生产补丁已由主线等价包含；以保留主线内容的祖先合并记录集成关系，避免回退远程更新、正式打包门禁和后续文档。
- `media_kit_video 2.0.1` 隔离迁移正常合入主线，保留固定归档、稳定纹理 descriptor 补丁、架构合同、Profile 压力入口和 FFmpeg 缩略图 A/B 工具。
- 合并验证通过后删除 `codex/auto-remove-missing-unreadable-clean` 与 `codex/media-kit-2-migration-experiment` 两个远程分支。

## 2026-07-24 · 正式打包分支集成与 Debug 门禁

- 新增正式打包前远程分支检查：待打包提交必须等于 `origin/master`，其它远程分支必须已经合入主线或达到补丁等价。
- 对仍有独有提交的分支，在隔离临时 Worktree 中按分支名稳定顺序累计试合并；发现冲突或未审查提交即阻断，不污染真实工作树，也不从临时结果直接打包。
- Windows 与 macOS 打包 job 统一依赖门禁；门禁还会执行全量测试、静态分析、Windows Debug 构建和 10 秒启动存活检查。
- 当前两个保留的远程开发分支仍未完全进入主线，其中自动清理分支与主线文档冲突，因此门禁会正确拒绝当前正式打包。
- 门禁验证发现并补齐媒体卡片菜单测试替身缺失的自动清理接口；focused test 与全量 272 项测试、静态分析、Windows Debug 构建及真实窗口启动均通过。

## 2026-07-25 · 0.2.1 正式构建

- 应用版本提升为 `0.2.1+3`，汇总 `v0.2.0` 之后已进入 `master` 的媒体纹理稳定性、开源许可和无效媒体记录自动清理改进。
- 正式构建沿用显式未签名发布门禁：Windows 安装器未进行 Authenticode 签名，macOS DMG 未进行 Developer ID 签名或 Apple 公证，发布资产附带 SHA-256 校验文件。
- 未修改 SQLite schema、标签/过滤语义、filtered queue、`PlayerBackend` contract、缩略图/媒体详情队列或用户数据。
...
- 组件存在或组件单测通过不再视为功能可用证据；关键行为必须具有页面/Route 级挂载或可达性保护，并按 UI 规则完成真实点击。证据缺失、删除未授权或独立验证拒绝时禁止提交推送。
- 生产或真实窗口发现的未授权删除统一升级为 Level 3 independent；Agent Eval 新增只增不删事故用例及指定验证记录硬门，第二轮 N=5 达到 5/5、平均 100 分、`stable=true`。
- 播放器增加隐藏态细进度挂载合同测试，专门捕获组件仍存在而真实 `PlayerPage` 挂载消失；未改变播放器运行时、filtered queue、后端、缓存、播放设置或用户数据。

## 2026-07-22 · 恢复播放器隐藏态细进度

- 精确历史定位发现提交 `5271f63` 在重排控制层时误删了 `PlayerPage` 中的隐藏态进度挂载，虽然 3px 组件与孤立测试仍存在，真实播放器已不再显示。
- 恢复控制条之外的独立底部 Stack 层：完整控制条显示时细进度淡出，3 秒自动收起后细进度淡入；它只绘制播放比例，不扩大点击区域，也不处理视频帧。
- `flutter analyze`、完整 264 项测试和 Windows Debug build 通过；真实 Debug 两态点击确认底边进度保留、控制条可从底部正常唤回，无遮挡、错位、溢出或队列抖动。
- 未修改 filtered queue、`PlayerBackend`、缓存、标签语义、播放设置或用户数据。
...
- 媒体卡悬停预览在初始化和 Player 构造失败时都会释放资源、清除 loading 并允许后续重试，不再永久停在加载态。
- 播放器打开阶段复用媒体库已验证缩略图衔接原生纹理；open 成功后保留 500ms 接管窗口，只有超过 800ms 的真实慢打开才展示 loading，损坏文件失败面板保持原语义。
- 新增初始化失败重试、首帧后单次预热、悬停 loading 复位、快速首播遮罩和纹理接管占位测试；未修改 SQLite、标签查询、filtered queue、PlayerBackend contract、缩略图调度或用户数据。
- `flutter analyze`、6 项聚焦回归、完整 263 项测试和 Windows Debug build 通过；真实启动预热约 210ms，悬停连续出画，首播占位与原生视频帧接管均无 loading 或黑屏。

## 2026-07-22 · 暗部增强闭环与 HDR 能力正式化

- “视频画质与增强”新增默认关闭的暗部细节增强；仅对明确 SDR、1080p 及以下且正在硬解的会话生效，4K、软解或传递函数未知时保持关闭。
- 保守 gamma 曲线与自动去块、`hqdn3d` 时空降噪和锐化合成单条 `vf` 快照；独立会话压力保护在掉帧、缓冲或停滞时自动回滚，不改写用户开关。
- HDR 开关从用户可见的“单项实验”文案收敛为“HDR 动态映射”，仍必须通过 HDR 源、活动 adapter LUID 和 Compute 门槛，并保留自动会话回滚。
- 删除设置页内部“画质增强路线”卡，避免把开发状态当成用户功能；保留真实解析、诊断和可操作开关。
- 最终 SDR 开/关两态各 60 秒均为 0 掉帧、0 停滞，GPU Engine P95 均为 5.0%；黑位保持 `YMIN=16`，近黑平均从 43.6642 升至 45.4358。HDR 样本在 1 个新总掉帧后真实触发自动回滚。
- `flutter analyze`、258 项完整测试、Windows Debug build 与三态 MediaKit 固定样本通过；Debug 真实点击及两态截图确认无遮挡、溢出或状态歧义。

## 2026-07-22 · 全屏边缘播放列表开关与命中修复

- 播放器交互页将“全屏播放列表”的热区宽度、隐藏延迟和恢复默认按钮收敛为一个默认开启的“全屏边缘播放列表”开关；固定参数来自交互容错，不再要求用户调试实现细节。
- 未展开时右缘命中区固定为 32px；展开后保持区使用实际列表宽度并增加 12px 容错，离开后固定等待 450ms，降低高分屏边缘难命中和轻微抖动误隐藏。
- 列表展开后不再保留覆盖最右内容的透明边缘层，鼠标移动到屏幕最右侧时列表仍保持，且右侧条目可完整点击。
- 开关只控制鼠标边缘自动唤出；显式列表按钮继续可用。旧设置文件中的宽度与延迟参数保持可读兼容，但运行时改用固定验证值。
- focused test 覆盖布尔设置往返、默认恢复与实际 476px 列表保持边界；filtered queue、PlayerBackend、SQLite、缓存队列和用户数据未改变。
- `flutter analyze`、完整 255 项测试和 Windows Debug build 均通过，3 项显式真实媒体 benchmark 按设计跳过；Debug 真实点击覆盖开关关闭、开启、最右缘保持及离开收起，设置页和全屏列表无截断、遮挡、错位或溢出。

## 2026-07-22 · 播放器全屏返回与会话恢复

- 新增媒体库 Route 级播放器全屏会话状态，只在当前应用会话内记住下一次播放器是否恢复全屏，不进入播放设置或桌面窗口布局持久化。
- 播放器全屏点击返回时先退出系统全屏并最大化应用窗口，再执行反向 Route；主界面及随后打开的其他页面不再继承无标题栏全屏状态。
- 再次进入播放器会恢复会话内全屏；用户在播放器内主动退出全屏会清除该状态。普通窗口或最大化窗口进入、返回时完全沿用原窗口路径。
- 新增状态边界测试，覆盖普通最大化不触发、全屏返回保留和主动退出全屏清除；未修改 filtered queue、PlayerBackend、SQLite、缓存队列、播放设置或用户数据。
- `flutter analyze`、完整 255 项测试和 Windows Debug build 通过；Debug 真实点击覆盖最大化窗口正反路径、全屏进入、Esc 清除及再次窗口化进入。自动化接口拒绝鼠标侧键，直接全屏侧键返回保留为精确人工复测项。
...
- 新增 Windows integration test 与 `tool/run_gpu_capability_matrix.ps1`，可保存不含媒体路径的 JSON；实测矩阵记录在 `docs/history/qa/2026-07/player_gpu_capability_matrix_20260722.md`。
...
- 隔离 profile 固定 1080p 类 H.264 与 4K 类 HEVC 样本，分别运行 `auto-safe` 和 `no`，稳定段同时采集进程 CPU、GPU Engine、GPU committed、实际解码器、掉帧、AV 偏移和音视频停滞；完整口径与结果写入 `docs/history/qa/2026-07/player_quality_baseline_20260722.md`。
...
- 主界面“播放与继续观看”设置新增原始高清码流内存缓存、画面比例、Bicubic / Lanczos 高质量缩放和自动 / Limited / Full 输出色彩范围；旧设置文件安全回落到 Lanczos、自动范围和开启高清缓存。
- 播放会话显式设置 `hwdec-software-fallback=3`，自动硬解连续失败后允许回退软件解码；高质量缓存使用 96 MiB 前向、32 MiB 回看与 15 秒预读，关闭时收敛到 32 MiB、8 MiB 与 5 秒，原生 A/B 后端不再静默覆盖设置值。
- GPU 超分关闭后恢复用户选择的 Bicubic 或 Lanczos 基线；开启时仍使用已有 `ewa_lanczossharp` 路径，不把普通高质量缩放宣称为厂商 AI 超分。
- 播放诊断新增亮度/色度缩放器、输出电平、源色彩范围/矩阵/原色/传递函数与实际输出范围；既有 FFprobe 详情缓存、实际硬解、缓存、解码/输出/总丢帧诊断继续复用。
- 设置页只读展示第二阶段观感增强和第三阶段高级增强路线；去块、降噪、锐化、暗部增强、时域降噪、补帧、HDR 与 Compute 能力尚未达到设备检测和流畅度门槛，因此未提供虚假可用开关。
- 精确 Debug 真实点击确认设置入口、Bicubic/Lanczos 切换恢复、11176 项队列滚动和持续播放；诊断为 `d3d11va-copy`、Lanczos、自动输出、源 `limited / BT.709 / BT.1886`，解码/总丢帧为 0、缓存约 111 秒。
- 完整 238 项测试通过，3 项显式 benchmark 跳过，`flutter analyze` 与 Windows Debug 构建通过；未修改 SQLite、标签过滤、filtered queue、稳定身份或用户数据。

## 2026-07-22 · 播放器控制显隐、全屏覆盖队列与快进档位

- 控制条首次进入默认显示，3 秒无交互后自动收起；收起后仅鼠标进入底部进度区域时重新显示，进出使用 Apple 式淡入/位移动效并遵循 reduced motion。
- 全屏视频列表改为根层固定覆盖，不再挤压、缩放或逐帧重排视频纹理；面板铺满当前窗口高度，以 320ms 淡入和短距离右滑进入、更短动画退出，队列滚动仍只构建可见项。
- 更多播放设置新增 5 / 10 / 15 / 30 / 60 秒离散快进快退滑动档位，旧设置缺字段或异常值时回到 5 秒；按钮和快捷键统一使用当前档位。
- 前进/后退反馈从画面中央 HUD 改为左上角轻量文字水印；键盘连发期间只显示一次，避免持续遮挡视频内容，其它快捷键反馈保持原位置。
- 真实 Windows 点击发现设置页内部新旧复杂内容树交叠会与视频纹理共同触发 Flutter 引擎访问冲突；二级设置页改为直接替换内容树后，“更多播放设置”可连续打开并稳定停留，弹层开合动画保留。
- 1249×714 与 2560×1440 实窗完成控制条自动隐藏、快进水印、更多设置、全屏队列开合及 11176 项队列滚动；视频持续播放且画面缩放不跳变。低分辨率超分开关两态诊断确认 `lanczos` / `ewa_lanczossharp`，测试后设置恢复关闭。
- 完整 238 项测试通过，3 项显式 benchmark 跳过，`flutter analyze` 与 Windows Debug 构建通过；未修改 SQLite schema、`FilterQuery` / `TagQueryService`、filtered queue、`PlayerBackend`、缓存队列或用户数据。
...
## Architecture Baseline 0.5.132

- 设置 Route 外壳和缓存诊断卡装配迁为 82/69 行 settings presentation 叶节点，
  `library_page.dart` 预算从 4487 行降到 4443 行。
- 新叶节点只接收首页/标题/刷新显隐、缓存统计快照及回调；section 状态、缓存读取与维护
  controller、Repository 写入、命令反馈和刷新生命周期仍由页面唯一拥有。
- focused 架构/Widget 测试共 252 项、完整测试 447 项通过（3 项 benchmark 按设计
  跳过）；静态分析、Windows Debug build 和 EXE 启动通过。Computer Use 原生管道
  缺失，设置缓存二级页真实点击与截图保留为人工复测项。
...
- 原始码流缓存、播放画质/流畅度、删除文件设置和缓存失败状态/测试容器迁为
  ...
  5391 行降到 4686 行。
- 新架构守卫要求四个叶节点只发出设置/动作意图，不持有页面 controller、缓存维护命令、
  媒体库服务、筛选、扫描或播放队列；原 Key、确认、撤销、危险提示和忙碌门禁保持不变。
- focused 254 项、完整 444 项测试、静态分析、Windows Debug build 和 EXE 启动通过；
  3 项 benchmark 按设计跳过。Computer Use 原生管道缺失，设置二级页真实点击与截图
  保留为人工复测项。

## Architecture Baseline 0.5.129

- sidebar 容器、折叠轨道、品牌区和桌面拖拽滚动行为拆为 418/213/121/16 行组件，
  top bar 搜索与筛选状态拆为 241/464 行组件；`library_widgets.dart` 从 3348 行降到
  1917 行，全部新增组件低于 500 行。
- 启动 `library_page.dart` 治理，把播放解码器/渲染器下拉迁到 367 行的设置叶节点，
  页面从 5747 行降到 5391 行；搜索输入链、筛选状态、导航、播放设置确认与撤销路径
  保持不变。
- focused 252 项、完整 443 项测试、静态分析、Windows Debug build 和 EXE 启动通过；
  3 项 benchmark 按设计跳过。Computer Use 原生管道缺失，侧栏、顶栏和播放设置真实
  鼠标路径与截图保留为人工复测项。

## Architecture Baseline 0.5.128

- 侧栏通用条目、左右面板转场和结果视图切换器迁为 237/52/221 行独立叶节点，
  `library_widgets.dart` 从 3819 行降到 3348 行；原导航、动画、`ValueKey`、无障碍
  语义和页面状态 owner 不变。
- 架构合同新增 8 个 1000+ 行 presentation 文件的有序强制治理清单；新增叶节点继续
  受 200/500/1000 行门禁和只降不升预算保护。
- focused 248 项、完整 443 项测试、静态分析、Windows Debug build 和 EXE 启动通过；
  Computer Use 原生管道缺失，侧栏开合与视图切换真实鼠标截图保留为人工复测项。

## Architecture Baseline 0.5.127

- 对齐 Effective Dart、Flutter 职责分离与 Google 小变更/评审标准；新增 presentation
  文件项目本地 200 行最佳实践、500 行警戒线与 1000 行强制重构线。现有超标文件进入
  只降不升预算，新超标文件由架构合同直接拒绝。
- 最近播放和标签编辑从 `library_widgets.dart` 拆为独立叶节点，聚合文件减少 758 行并
  清除无关播放设置、播放进度和缩略图依赖；页面回调、筛选和 filtered queue 不变。
- focused 248 项、完整 443 项测试、静态分析、Windows Debug build 与正式 EXE 启动
  通过；Computer Use 原生管道缺失，真实鼠标路径和截图保留为人工复测项。

## Architecture Baseline 0.5.126

- 16 个单元测试和 9 个 integration test 从 `src/app.dart` 迁到具体模块 import；消费者
  归零后删除万能兼容导出面。
- 架构合同同时保护 production、test 与 integration_test 不得重新引入 barrel；测试
  行为、应用入口、页面挂载、schema、筛选和播放队列均未改变。
