...
> 本文件是截至 2026-07-30 的路线图演化归档；当前优先级见仓库根目录 `ROADMAP.md`。
...
- [x] 媒体打开拆为一次引擎快照、一次滤镜基线和一次显示快照，移除 open 前后重复提交。
- [x] 串行隔离跨媒体 GPU 属性任务，并以打开代次和路径阻止过期结果发布。
- [x] GPU 缩放、压缩/暗部滤镜及 HDR→SDR 映射增加属性回读和真实会话状态。
- [x] 正式 MediaKit 诊断移除 NVIDIA 原生探测；child HWND 只保留显式 QA 门禁。
- [ ] 如未来重新评估 NVIDIA SDK，只能新增独立原生 Texture 扩展边界、许可门禁与非
  NVIDIA 回退；不得恢复伪播放器选择或在 Dart 层按显卡名称宣称激活。
...
- [x] 按九个一致性边界拆分页面协调逻辑，并把设置状态 owner 与纯展示工作区分开；
  `library_page.dart` 从 4293 行降到 750 行，全部新文件低于 500 行，退出强制重构清单。
- [ ] 下一批依次治理播放器页面、
  视频结果、播放器队列侧栏、标签发现面板、
