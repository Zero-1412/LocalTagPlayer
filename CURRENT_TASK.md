...
架构版本状态：`Architecture Baseline 0.5.36` 已完成。
...
结果：通过；132 项测试通过，3 项显式烟测/基准跳过。

## 最近完成

- 只读审计确认收藏一直持久化到 SQLite；正式库当前仅剩 1 条收藏，而 7 月 13 日快照保留 3 条旧收藏。正式视频索引在 7 月 15 日整批重建并产生新 `videoId`，旧收藏和大部分播放记录因此没有迁移到新身份；本轮未自动改写用户数据库。
