...
- 标签详情把显示名称、别名、标签组、排序值、两个开关、保存、批量和高风险入口锁定为视觉阅读顺序；标签组下拉使用完整字段宽度、有限菜单高度和统一圆角，浮层保持在触发字段 8px 阴影安全区内。
- 标签详情分区改用真实 `Material` 表面，恢复 `SwitchListTile` 的 hover/focus/ink；高风险入口改为“检查合并影响/检查删除影响”，只读反馈层明确“本次未执行”，默认焦点落在“返回标签详情”。
- manual 标签编辑器新增未保存提示和移除标签 tooltip；添加或移除只改变弹窗内临时选择，取消仍不会写入标签关系。
- focused tests 与完整 224 项测试通过，3 项显式 benchmark 跳过；`dart format`、`flutter analyze` 和 Windows debug build 通过。最新构建的 150% 实窗连续点击因检测到用户输入而停止，待窗口空闲后按“标签中心 → 详情下拉 → 高风险反馈 → 视频标签编辑器”补拍，且只打开后取消。
