...
- 顶部搜索表面新增输入宽度保护：筛选 chips 只使用输入区之外的剩余预算，两个常用标签可以同时显示，更多标签优先折叠为数量；真实 `TextField` 在 expanded 下至少保留 236px，不再因标签增加而被压缩成狭窄输入缝隙。
- “多选”改为与排序、网格/列表切换相同的 48px 高度、10px 圆角、深色表面和统一描边，不再使用视觉高度偏低的默认 `OutlinedButton`。
- 网格/列表滑块改用独立连续动画控制器：点击先让底块从当前位置平滑移动，动画稳定后再提交较重的结果区重布局；快速连续点击可中途反向，无关父级重建也不会把滑块拉回旧状态。完整 154 项测试（3 项显式 benchmark 跳过）、`flutter analyze` 和 Windows debug build 通过，新 Debug EXE 已启动且响应正常。当前运行时未暴露 `computer-use` 所需 Windows 控制通道，保留“选择原神/雷神检查搜索宽度 → 对比多选与相邻控件高度 → 快速连续点击视图滑块并截图”的人工复测路径。

- 媒体库顶部改为单层极简工具栏：删除视频区上方独立筛选栏，标签条件以可移除 chip 融入最大约 650px 的搜索区域，筛选后视频数量固定跟随搜索结果状态；宽屏保留自然留白，medium 窄行使用紧凑排序图标且无横向溢出。
- “标签中心”从顶部移入左侧功能栏，折叠态继续提供 tooltip；紧凑布局因没有常驻侧栏而保留图标入口。排序、多选和网格/列表切换保持在同一行右侧，不增加第二行。
- 进入多选后整条顶部区域替换为“已选择 N 项 / 删除 / 取消”，圆形复选框继续承担全选/取消全选；退出后恢复原搜索文本、筛选 chips、结果数量和排序状态。完整 153 项测试（3 项显式 benchmark 跳过）、`flutter analyze` 和 Windows debug build 通过，新 Debug EXE 已启动且响应正常。当前会话缺少可调用的 Windows 点击/截图通道，保留“左侧选择标签 → 检查 chip 与数量 → 进入多选 → 选择/全选 → 打开删除确认并取消 → 退出恢复”的人工截图复测路径。
