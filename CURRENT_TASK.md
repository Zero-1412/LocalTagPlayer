...
- 最新 Debug EXE 已启动并唯一定位；窗口激活时 Windows 控制检测到目标窗口正在接收用户输入，因此已停止抢占，未执行真实点击或截图。仍需在窗口空闲时复测：进入播放器 → 播放/暂停与 seek → 列表/详情往返 → 左滑快速反向 → 全屏 → 右缘唤出/收起覆盖队列 → Esc → 返回媒体库，并检查位置、遮挡、对齐、溢出、对比度和状态反馈。
