...
- 完整 197 项测试通过，3 项显式 benchmark 跳过；`flutter analyze` 与 Windows debug build 通过。正式窗口验收时发现当前可见窗口仍是构建前驻留内存的旧进程；尝试关闭重启时 Windows 控制检测到用户正在操作目标窗口，已按规则停止输入。待窗口空闲后执行：关闭旧实例 → 启动最新 Debug EXE → 100%/125%/150% 主界面截图 → 搜索/清除 → 展开标签/选择/清空 → 打开视频/返回并确认筛选保持。
