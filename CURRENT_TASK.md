...
- 验证进度：新增分组 focused tests 与侧栏完整/折叠可达性测试通过；待完成 `flutter analyze`、
  `flutter build windows --debug` 和独立只读 diff 审查。
- 下一步：完成构建与运行时挂载检查；若需要识别重新编码而非重复下载，再单独设计更严格的媒体详情相似度规则，
  不在本次轻量指纹候选中静默扩展。
