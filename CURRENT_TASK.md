...
## 活跃任务：media_kit_video 2.0.1 独立迁移实验

- 实验分支已把 `media_kit_video` 升到 2.0.1，但继续固定下载 SHA256，并在 Windows 构建期替换 `video_output.cc`；GPU 与软件纹理回调捕获稳定 descriptor，销毁后返回空指针，直到 `UnregisterTexture` 才移除 map 所有权。
- Flutter SDK 保持 3.44.4、Visual Studio 保持 2022 17.13.6。Windows Profile 基线通过 75 秒真实媒体库压力测试：3 轮播放器打开/队列滚动/seek/全屏/退出，1227 帧，total P95 13.023ms，超过 33ms 共 8 帧，播放掉帧与音视频停滞均为 0。
- FFmpeg 8.1.2 缩略图 A/B 暂不进入产品：1080p/4K H.264 两样本中，现有软件路径均快于 D3D11 硬解回读和 D3D12 硬缩放；`scale_d3d11` 在当前构建与设备上不能创建目标纹理。
- 下一步：独立只读复核补丁挂载、受保护行为与最终 diff；通过完整验证后只提交并推送实验分支，不合并到主开发分支。
