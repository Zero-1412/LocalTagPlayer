...
- 150% 媒体库入口截图暴露卡片标题受固定高度裁切，属于后续媒体库响应式任务，不混入播放器修复。100% 播放器已置前等待人工“左滑展开 → 回拖一半 → 快速反向 → 停在展开态”，当前尚未收到完成信号，不能把左滑真实截图记为通过。
...
- 当前 Windows 控制的拖拽无法让队列左滑隐藏动作层保持展开，三次尝试均只改变选中项，因此快速反向仍以 focused widget 的中间态、终态与吸附断言作为替代证据；单键 `F` 被中文输入法候选截获，UI 全屏按钮路径已通过。播放器返回已在受控调试会话中复验通过，左滑终态仍待人工配合截图。
- 本轮未修改 SQLite schema、`FilterQuery` / `TagQueryService`、filtered queue 内容与顺序、`PlayerBackend`、缩略图/媒体详情队列、稳定身份或用户数据。
