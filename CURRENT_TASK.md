...
- 当前 Windows 控制的拖拽无法让队列左滑隐藏动作层保持展开，三次尝试均只改变选中项，因此快速反向仍以 focused widget 的中间态、终态与吸附断言作为替代证据；单键 `F` 被中文输入法候选截获，UI 全屏按钮路径已通过。点击播放器左上返回后目标进程退出、未回到媒体库，窗口句柄与只读进程检查均确认进程消失，列为待修复的真实返回路径缺陷；不得把该项记录为通过。
