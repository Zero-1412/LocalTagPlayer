...
- 标签详情把显示名称、别名、标签组、排序值、两个开关、保存、批量和高风险入口锁定为视觉阅读顺序；标签组下拉使用完整字段宽度、有限菜单高度和统一圆角，维护主题补齐深色 `canvasColor` 与紫色交互态，避免弹层退回全局亮色画布。
- 标签详情分区改用真实 `Material` 表面；维护主题为 `SwitchListTile` 固定浅色滑块、深色轨道和单层 hover/focus 反馈，消除黑色滑块、整轨紫色覆盖和双重圆环；高风险入口改为“检查合并影响/检查删除影响”，只读反馈层明确“本次未执行”，默认焦点落在“返回标签详情”。
- manual 标签编辑器新增未保存提示和移除标签 tooltip；添加或移除只改变弹窗内临时选择，取消仍不会写入标签关系。
- focused tests 与完整 226 项测试通过，3 项显式 benchmark 跳过；`dart format`、`flutter analyze` 和 Windows debug build 通过。125%/150% 真实窗口已经连续完成“标签中心 → 详情下拉 → 开关 hover → 高风险反馈并取消 → 视频标签编辑器未保存提示并取消”；临时添加的 `3d` 标签已取消，没有写入用户数据。
- 连续验收额外暴露并修复两处缩放问题：150% 下标签组字段会在展开前移入可视区中段，三个选项均完整留在窗口内；高风险反馈通过维护页深色弹窗表面承接 Navigator 路由，不再退回浅色画布。两档开关 hover/selected/off 均保持单层状态反馈。
- 未修改 SQLite schema、`FilterQuery` / `TagQueryService`、filtered queue、备份/缓存后端、标签来源语义、稳定身份或用户数据。

## 2026-07-19 删除偏好、全局返回与快捷键录制

- 删除确认统一记忆“同步移入回收站”最终状态，单条、批量和播放器队列共用同一偏好；弹窗增加“不再提示”，关闭提示后按已保存最终状态直接执行，设置写入失败会中止删除，文件动作仍只经过 `FileSystemAdapter.moveFileToTrash`。
- 设置首页“数据与维护”增加“删除文件”二级页，可分别控制删除提示和回收站默认动作；关闭提示时显示明确危险状态。删除确认层固定使用维护页深色主题，修复 Navigator 浅色基线导致的浮层割裂和低对比文字。
- 非主路由统一支持鼠标返回侧键与可配置“返回上一页”快捷键；播放器在普通窗口使用同一绑定，全屏仍保留 Esc 安全退出。设置二级页优先返回设置首页，快捷键录制释放焦点后仍可继续返回。
- 快捷键设置改为点击后直接监听常用单键或 Control / Alt / Shift 组合键；与其它绑定或播放器保留动作冲突时原位红字提示，不交换、不覆盖。设置写入失败会恢复旧绑定。
- focused tests、完整 221 项测试通过，3 项显式 benchmark 跳过；`flutter analyze` 与 Windows debug build 通过。1249×714 真实窗口覆盖删除记忆、冲突录制、录制后返回、深色删除弹窗及 100%/150% 设置布局；没有确认任何删除，偏好恢复为“提示开启、回收站关闭”，应用恢复正常文字缩放并停在媒体库。
- 未修改 SQLite schema、`FilterQuery` / `TagQueryService`、filtered queue 内容/顺序、`PlayerBackend`、缩略图/媒体详情队列、stable identity 或用户标签/收藏数据。
...
- 猫咪焦点使用 26px 矢量轮廓、紫色渐变和最少面部细节，不加载位图、不触发媒体查询或列表重算；focused test 锁定主进度条不得再次静默回退为通用白色圆环。
- 控制条文件按钮 tooltip 明确为“在文件管理器中显示当前视频”，定位路径固定读取 `PlayerPlaybackController.currentItem`；即使队列选择停在其它条目，也不会错误打开未播放视频或媒体库根目录。
- 完整 213 项测试通过，3 项显式 benchmark 跳过，`flutter analyze` 与 Windows debug build 通过。真实窗口确认进度轨道无位移、裁切或溢出；资源管理器打开到当前视频目录并选中播放器标题对应的 `7月12日(4).mp4`。
- 真实窗口收尾时检测到用户输入，已立即停止自动化，不再抢占前台；本轮未修改标签、收藏或媒体文件。
- 未修改 SQLite schema、`FilterQuery` / `TagQueryService`、filtered queue 内容/顺序、`PlayerBackend`、缩略图/媒体详情队列或用户数据。

## 2026-07-18 Apple UI Phase 2 播放队列操作层与非全屏顶栏联动

- 队列左滑操作层在进度完全收起时不再参与绘制与命中，解决真实 Windows 像素约束下收藏/删除图标从卡片右缘泄出的问题；条目切换为当前播放项时同步把旧左滑进度归零，后续主动左滑收藏/删除仍可用。
- 非全屏宽屏播放器把标题栏与右侧队列绑定为同一结构状态：队列展开时标题栏常驻，队列折叠时同步收起；指针进入窗口顶部 64px 热区后以短动画显示，移开即收回。全屏覆盖队列逻辑不变，`accessibleNavigation` 下保留返回入口可达。
- 根级指针坐标只在热区布尔值变化时更新状态，避免标题栏中间帧丢失 exit，也不增加队列遍历、播放器查询或每像素重建。focused tests 覆盖隐藏操作层、播放身份切换、顶栏状态、热区、全屏与辅助导航。
- 完整 212 项测试通过，3 项显式 benchmark 跳过，`flutter analyze` 与 Windows debug build 通过。1249×714 真实窗口确认播放项无重叠、队列/顶栏折叠、顶部悬停、移开收回和同步展开均无错位、裁切或残影。
- `build/qa/apple_phase3_2026-07-18/` 已保存媒体库中途隐藏、回顶入口、回顶终态、备份页、播放项无重叠、队列/顶栏三态和标签编辑器共 9 张连续截图；标签编辑器以 Esc 取消，未改用户数据。
- 140ms 恢复延迟在 Windows 注入滚轮下没有中途闪回或停住误恢复，当前无需调时；自动化无法生成真实 Precision Touchpad 惯性，硬件触控板手感仍需人工补一轮，且只允许微调时长。
- 未修改 SQLite schema、`FilterQuery` / `TagQueryService`、filtered queue 内容/顺序、当前播放索引、`PlayerBackend`、缩略图/媒体详情队列或用户数据。
