...
- focused tests 与完整 226 项测试通过，3 项显式 benchmark 跳过；`dart format`、`flutter analyze` 和 Windows debug build 通过。125% 首次实窗抓到标签组弹层亮色画布并已修复；重建后继续控制时检测到人工输入，按安全规则停止，125%/150% 仍待窗口空闲后按“标签中心 → 详情下拉 → 开关 hover → 高风险反馈并取消 → 视频标签编辑器未保存提示并取消”连续复拍。
