...
- 设置页“删除文件”新增默认开启的“自动移除缺失或不可读视频”；开启后立即异步清理当前记录，后续扫描完成后复用同一清理入口。
- missing 只认成功扫描 root 后持久化的 `isMissing`；路径暂时不可访问但未标记 missing 时保留。不可读只认当前存在但不是普通文件或无法打开句柄。
- 清理仅删除主库视频行、标签关系和防自动恢复的依赖备份，不调用回收站或磁盘删除接口，不删除任何文件或文件夹。
- 当前进入完整构建、真实设置路由点击和独立只读审查阶段。
